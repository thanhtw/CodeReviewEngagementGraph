...
export function renderCooccurrenceChart(matrix, labels) {  // 要改成
    ...
    ).filter(item => item.value > 0); // 只顯示有共現的數據
            ...
            plugins: {
                tooltip: {
                    callbacks: {
                        ...
                        label: (context) => {
                            return `共現次數: ${context.dataset.data[context.dataIndex].value}`;
                        }
                    }
                }
            }
...
export function renderConditionalProbChart(probMatrix) {
    ...
    if (!ctx) {
        console.error('找不到 probabilityChart 元素');
        return;
    }
...
export function renderCombinedProbChart(selectedData, labels) {
    ...
    if (!ctx) {
        console.error('找不到 combinedProbabilityChart 元素');
        return;
    }

    if (combinedProbabilityChart) {  // 銷毀舊圖表
            combinedProbabilityChart.destroy();
            combinedProbabilityChart = null;
        }
    
    if (!selectedData || !Array.isArray(selectedData)) {  // 確保數據存在
    ...
    const dynamicMax = Math.max(0.3, maxValue * 1.5); // 至少0.3，或最大值的1.2倍

    console.log('組合條件機率結果:', combinedProbs);
    console.log('動態最大值:', dynamicMax);
...
export function calculateTripleCooccurrence(data, labels) {
    ...
    console.log('調試 - 統計結果:', stats); // 重要調試信息
    return stats;
}
...
export function calculateAdvancedConditionalProb(stats, labels) {
    ...
    // 生成正確的 key
    const sortedLabels = [...labels].sort(); // 確保與計算時的順序一致
    const relConstKey = ['constructive', 'relevance'].sort().join('→');
    const concConstKey = ['concreteness', 'constructive'].sort().join('→');
    const relConcKey = ['concreteness', 'relevance'].sort().join('→');
    const tripleKey = sortedLabels.join('→');
    
    console.log('調試 - Keys:', { relConstKey, concConstKey, relConcKey, tripleKey });
    console.log('調試 - Double stats:', stats.double);
    console.log('調試 - Triple stats:', stats.triple);
    ...
    console.log('調試 - 計算結果:', { singleProbs, combinedProbs });
...
export function calculateDetailedConditionalAnalysis(selectedData, labels) {
    ...
    // 計算條件機率
    // 1. 單條件：P(constructive | relevance)
    ...
    // 2. 雙條件：P(constructive | relevance AND concreteness)
...
export async function generateHwLabelChart() {
    try {
        ...
        // 載入3標籤資料
        const response = await fetch('../function/3labeled_processed_totalData.json');
        if (!response.ok) {
